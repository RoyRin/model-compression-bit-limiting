...
def normalize_to_base_name(name: str) -> str:
    name = (name.replace("Chat", "").replace("instruct", "").replace(
        "code-llama",
        "CodeLlama").replace("deepseek-coder", "DeepSeek-Coder").replace(
            "gpt-4-turbo",
            "GPT-4-Turbo").replace("starcoder", "StarCoder").replace(
                "--v0.1", "").replace("-base", "").replace("-preview",
                                                           "").strip("-"))
    # ${NUM}b -> ${NUM}B
...
def load_groups_from_directory(result_dir: str, norm: bool = False):
    ...
    for result_json in os.listdir(result_dir):
        ...
        if not (type == "instruct" or
                (type == "base" and model_id == "StarCoder2")):
            continue
...
def main(result_dir: str, norm: bool = False, latex: bool = False):
    ...
    for k, (model, group) in enumerate(groups.items()):
        ...
        score_matrix_diff = [[(score[0] - score[1]) for score in score_list]
                             for score_list in score_matrix]
        ax: plt.Axes = axs[k]
        cmap = LinearSegmentedColormap.from_list("rg", ["r", "w", "lime"],
                                                 N=256)
