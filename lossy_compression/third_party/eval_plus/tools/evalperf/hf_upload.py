...
def main(path, overwrite=False):
    ...
    to_drop = [
        task["task_id"] for task in data
        if task["task_id"] not in evalplus_datasets
    ]
    print(
        f"Removing {len(to_drop)} tasks that are not in the latest EvalPlus dataset"
    )
    print(to_drop)
    ...
    print(
        f"Uploading dataset with tag {version} to Hub... Please enter to confirm:"
    )
    input()
