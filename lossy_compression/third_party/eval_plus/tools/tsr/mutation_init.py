...
def prepare_mutants(mutation_dir: str, dataset: str):
    ...
    for task_id in track(task_ids, "Generating mutants"):
        ...
        if any(
                map(lambda filename: filename.startswith("m"),
                    os.listdir(task_dir))):
            # already have mutants
            continue
        # Make groundtruth
        groundtruth_code = (problems[task_id]["prompt"] +
...
def collect_mutation_info(eval_path: str,
                          dataset: str) -> Dict[str, Dict[str, List[Any]]]:
    mutation_info = {task_id: {} for task_id in get_task_ids(dataset)}
    assert os.path.isfile(
        eval_path), f"mutation testing result file {eval_path} missing!"
