...
def greedy_cover(task_id: str, tests: Dict[str, List[Any]],
                 exclude_model: str) -> Tuple[str, List[str]]:
    q, U = [], set()
    ...
    with ProcessPoolExecutor(max_workers=32) as executor:
        ...
        for task_id in task_ids:
            if type == "sample":
                ...
                with open(os.path.join(sample_dir, f"{path_task_id}.pkl"),
                          "rb") as f:
                    td = pickle.load(f)
...
def get_coverage_set_cover(coverage_dir: str, exclude_model: str,
                           dataset: str) -> Dict[str, List[str]]:
...
def get_mutation_set_cover(mutation_dir: str, exclude_model: str,
                           dataset: str) -> Dict[str, List[str]]:
...
def get_sample_set_cover(sample_dir: str, sample_eval_dir: str,
                         exclude_model: str,
                         dataset: str) -> Dict[str, List[str]]:
    collect_sample_info(sample_dir, sample_eval_dir, dataset)
    return parallel_greedy_cover(None,
                                 exclude_model,
                                 "sample",
...
def gen_report(set_cover_info: Dict[str, List[str]], sample_eval_dir: str,
               model: str):
    tsr_dict = {"ntests": compute_avg_test(set_cover_info), "pass@1": 0}
    model_path = os.path.join(sample_eval_dir, f"{model}_temp_0.0",
                              "eval_results.json")
...
def dump_humaneval_plus_mini(set_cover_info: Dict[str, List[str]],
                             mini_path: str):
    new_problems = []
    for task_id in task_ids:
    ...
    write_jsonl(os.path.join(mini_path, "HumanEvalPlus-Mini.jsonl"),
                new_problems)
...
def main(flags):
    ...
    mutation_set_cover = get_mutation_set_cover(mutation_dir, exclude_model,
                                                flags.dataset)
    sample_set_cover = get_sample_set_cover(sample_dir, flags.sample_eval_dir,
                                            exclude_model, flags.dataset)
    merged_set_cover = merge_set_cover(coverage_set_cover, mutation_set_cover,
                                       sample_set_cover)

    if flags.model != "ALL":
        final_report = dict()
        # Stage 1: Coverage min set cover
        final_report["coverage"] = gen_report(coverage_set_cover,
                                              flags.sample_eval_dir,
                                              flags.model)
        # Stage 2: Mutation min set cover
        final_report["mutation"] = gen_report(mutation_set_cover,
                                              flags.sample_eval_dir,
                                              flags.model)
        # Stage 3: Sampling min set cover
        final_report["sample"] = gen_report(sample_set_cover,
                                            flags.sample_eval_dir, flags.model)
        # Stage 4: All
        final_report["full"] = gen_report(merged_set_cover,
                                          flags.sample_eval_dir, flags.model)
        with open(os.path.join(flags.report_dir, f"report_{flags.model}.json"),
                  "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample_eval_dir",
                        type=str,
