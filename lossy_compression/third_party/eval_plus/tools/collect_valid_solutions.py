    ...
    class FunctorVisitor(ast.NodeVisitor):
        ...
        def visit_FunctionDef(self, node):
            ...
            if self._temp_found_functors:
                self.results.append(
                    (self.current_function, self._temp_found_functors))
        ...
        def visit_Call(self, node):
            if isinstance(node.func,
                          ast.Name) and node.func.id in self.target_functors:
...
def test_solutions(dataset: str, solutions: List[str], task: Dict,
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = [
            executor.submit(correctness_check, index, solution, dataset, task,
                            expected_output)
...
def script(sample_dir: str,
    ...
    for task_id, task in tqdm(problems.items()):
        ...
        correct_solutions = test_solutions(dataset, solutions, task,
                                           expected_output[task_id])

        # clean solutions to remove print statements and format it
        correct_solutions = [
            void_calls(solution, ["print"])[0]
        ...
        if previous_solutions and task_id in previous_solutions:
            correct_solutions = deduplicate(correct_solutions +
                                            previous_solutions[task_id])
