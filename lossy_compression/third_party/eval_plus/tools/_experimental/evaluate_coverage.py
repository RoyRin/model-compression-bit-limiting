...
def test_code_coverage(code: str,
    ...
    p = multiprocessing.Process(target=safety_test,
                                args=(code, inputs, entry_point))
    p.start()
    p.join()
    safe = p.exitcode == 0
    if p.is_alive():
...
def test_solution_coverage(
    ...
    * mode: {"branch"}, will support "line" for coverage-guided LLM test generation
    ...
    if "HumanEval" in dataset:
        ...
        code = problem["prompt"] + (impl if impl != "canonical" else
...
if __name__ == "__main__":
    ...
    if args.mode == "branch":
        for i in range(0, 164):
            ...
            per = 1.0 if len(
                branch) == 0 else len(branch_covered) / len(branch)
            if per != 1.0:
                print(i, per, len(branch_covered), len(branch))
    else:
        for i in range(0, 164):
            task_id = f"HumanEval/{i}"
            annotated_code = test_solution_coverage(dataset="HumanEval",
                                                    task_id=task_id,
                                                    mode="line")
