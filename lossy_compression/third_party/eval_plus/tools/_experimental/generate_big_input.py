...
def main():
    ...
    for p in problems:
        ...
        big_input_path = os.path.join(HUMANEVAL_PLUS_BIG_INPUTS_PATH,
                                      f"{filename}.json")
        ...
        proc = multiprocessing.Process(target=input_generation,
                                       args=(inputs, signature, contract_code))
        proc.start()
        ...
        with open(
                os.path.join(HUMANEVAL_PLUS_BIG_INPUTS_PATH,
