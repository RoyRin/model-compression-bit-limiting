...
def execute_for_runtime(code: str, inputs: List, warmups: List,
                        entry_point: str) -> Union[str, float]:
...
def test_solution_runtime(
    ...
    if "humaneval" in dataset:
        ...
        impl = problem["prompt"] + (impl if impl != "canonical" else
        ...
        for input_list in inputs:
            ...
            for base_input_list in problem["base_input"]:
                if (hash(str(base_input_list)) != hash(str(input_list))
            ...
            sd = math.sqrt(
                sum((runtime - avg_runtime)**2
                    for runtime in runtime_list) / (RUN_REPEAT - 1))
            if sd < results[1]:
